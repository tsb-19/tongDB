...
void TokenStreamRewriter::insertAfter(const std::string &programName, size_t index, const std::string& text) {
  // to insert after, just insertData before next index (even if past end)
...
std::unordered_map<size_t, TokenStreamRewriter::RewriteOperation*> TokenStreamRewriter::reduceToSingleOperationPerIndex(
  ...
  for (size_t i = 0; i < rewrites.size(); ++i) {
    ...
    for (auto *iop : inserts) {
      if (iop->index == rop->index) {
        // E.g., insertData before 2, delete 2..2; update replace
        // text to include insert before, kill insertData
        ...
        // delete insertData as it's a no-op.
        delete rewrites[iop->instructionIndex];
        rewrites[iop->instructionIndex] = nullptr;
      }
    }
  ...
  for (size_t i = 0; i < rewrites.size(); i++) {
    ...
    // combine current insertData with prior if any at same index
    ...
    for (auto *prevIop : prevInserts) {
      if (prevIop->index == iop->index) { // combine objects
        ...
        // delete redundant prior insertData
    ...
    for (auto *rop : prevReplaces) {
      if (iop->index == rop->index) {
        ...
        delete rewrites[i];
        rewrites[i] = nullptr; // delete current insertData
        continue;
      }
      if (iop->index >= rop->index && iop->index <= rop->lastIndex) {
        throw IllegalArgumentException("insertData op " + iop->toString() + " within boundaries of previous " + rop->toString());
      }
    }
  }
